	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			float a1;
			float a2;
			a0 = (Vector2(p0.x() - p.x(), p0.y() - p.y()).cross(Vector2(p1.x() - p.x(), p1.y() - p.y()))) / 2;
			a1 = (Vector2(p1.x() - p.x(), p1.y() - p.y()).cross(Vector2(p2.x() - p.x(), p2.y() - p.y()))) / 2;
			a2 = (Vector2(p2.x() - p.x(), p2.y() - p.y()).cross(Vector2(p0.x() - p.x(), p0.y() - p.y()))) / 2;
			a0 = fabs(a0);
			a1 = fabs(a1);
			a2 = fabs(a2);
